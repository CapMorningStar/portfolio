...
let hasExperiencedIntroInSession = false;

export function IntroOverlay() {
  const [isVisible, setIsVisible] = useState(() => !hasExperiencedIntroInSession);
  ...
  useEffect(() => {
    if (hasExperiencedIntroInSession) {
      setIsVisible(false);
  ...
  const handleExit = (e?: React.MouseEvent | React.TouchEvent) => {
    ...
    if (isExiting) return;
    hasExperiencedIntroInSession = true;
    setIsExiting(true);
    document.body.style.overflow = 'auto';
