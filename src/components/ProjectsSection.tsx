      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredProjects.map((project) => {
          ...
          return (
            <div
              key={project.id}
              className="h-[380px] w-full [perspective:1400px]"
            >
              ...
              >
                {/* 1. FRONT OF CARD (Click anywhere to flip slowly in place) */}
                <div
                  ...
                  className="absolute inset-0 w-full h-full rounded-[2.2rem] bg-[#111111]/90 backdrop-blur-xl border border-white/10 hover:border-cyan-500/40 transition-colors p-7 flex flex-col justify-between cursor-pointer shadow-[0_10px_40px_rgba(0,0,0,0.5)] group overflow-hidden"
                >
                ...
                >
                  ...
                  <div className="relative z-10 my-auto py-1 space-y-2 overflow-y-auto pr-1">
                    {project.bullets.map((bullet, idx) => (
                      <div key={idx} className="flex items-start gap-2 text-[11px] text-gray-200 leading-snug">
                  ...
                  <div className="relative z-10 pt-3 border-t border-white/10 flex items-center justify-between gap-3">
                    <div className="flex flex-wrap gap-1 max-w-[55%]">
                      {project.tags.slice(0, 4).map((tag) => (
                        <span
                          key={tag}
                          className="px-1.5 py-0.5 rounded text-[8px] font-bold uppercase bg-[#1a1a1a] text-cyan-300 border border-white/5"
                        >
                          {tag}
                        </span>
                      ))}
